...
const printBookInfo = function ({ title, author, year = 'unknown' }) {
  console.log(`${title} by ${author} was published in ${year}`);
}
...
printBookAuthorsCount = function (title, ...author) {//...rest operator should be at the end 
  console.log(`The book "${title}" has ${author.length} authors. `);
}

printBookAuthorsCount('Algorithms', 'Robert', 'kevvin');
